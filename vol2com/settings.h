...
namespace vol2com
{
  ...
  public:
    ...
    QVariant get(const Section& section, const Key& key, const QVariant& defaultValue) const
    {
    ...
    template<>
    QVariant get(const QVariant& section, const QVariant& key, const QVariant& defaultValue) const
    {
      return m_settings.value(QStringLiteral("%1/%2").arg(section.toString(), key.toString()), defaultValue);
    }

    template <class Section=QString, class Key>
    int getInt(const Section& section, const Key& key, int min, int max, int defaultValue) const
    {
      const auto a = QVariant::fromValue(section).toString();
      const auto b = QVariant::fromValue(key).toString();
    ...
    EnumT getEnum(const Section& section, const Key& key, const EnumT& defaultValue) const
    {
