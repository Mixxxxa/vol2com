...
namespace
{
  ...
  /*!
   * @brief Precomputed B1 cache for final values calculation
   * @note Originaly calculated by the following formula 'i = round(pow(2, static_cast<double>(x * 10) / (BassLibWrapper::AvailableBands - 1)))'
   * @note At the moment precomputed values are used because std::pow will be constexpr with C++26
   * @todo Rewrite to constexpr when C++26 will be available
   */
  ...
  /*!
   * @brief Helper function for BassWASAPI. See BASS_WASAPI_Init docs for details
   */
  ...
  void CALLBACK WasapiNotifyProc(DWORD notify, DWORD device, void *user) noexcept
  {
    Q_UNUSED(user)
  ...
  int getLoopbackDevice(int deviceID)
  {
    ...
    if(BASS_WASAPI_GetDeviceInfo(static_cast<DWORD>(deviceID), &device))
    {
...
uint8_t BassLibWrapper::band(int band)
{
  ...
  {
    peak = 0;
    size_t b1 = ::B1Cache[static_cast<size_t>(x)];
    if (b1 <= b0)
      b1 = b0+1;
    for (; b0 < b1; ++b0)
      if (peak < m_fft[1+b0])
...
void BassLibWrapper::onBassEvent(const DWORD& event, const DWORD& device)
{
  ...
  case BASS_WASAPI_NOTIFY_FAIL:
    onDeviceFailed(static_cast<int>(device));
  }
}
